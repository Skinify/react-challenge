...
export default () => {
  ...
  if (
    currentQuest === allQuestions.length &&
    loading === false &&
  ...
  if (allQuestions.length > 0 && loading === false) {
    return (
      <Question
        questNumber={currentQuest}
        questReport={questReport}
        setReport={setReport}
        mistakes={mistakes}
        setMistakes={setMistakes}
        hits={hits}
        setHit={setHit}
        quest={allQuestions[currentQuest]}
        nextQuestion={() => setCurrentQuest(currentQuest + 1)}
      />
    );
  } else {
